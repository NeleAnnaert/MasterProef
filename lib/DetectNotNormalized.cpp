/*
 * DetectNotNormalized people in bed and see when they step out
 * Author: Nele Annaert
 */
...
DetectNotNormalized::DetectNotNormalized(cv::Mat img)
{
  bed.setValuesAuto(img);
}

DetectNotNormalized::DetectNotNormalized (int irbx, int irby, int irox, int iroy, int ilox, int iloy, int ilbx, int ilby)
{
  bed.setValues (irbx, irby, irox, iroy, ilox, iloy, ilbx, irby);
}

bool DetectNotNormalized::checkMovement(cv::Mat prev, cv::Mat current)
{
...
cv::Mat DetectNotNormalized::createMask(cv::Mat img)
{
  cv::Mat mask;
  int rows=img.rows;
  int cols=img.cols;
  ...
  for (int j=0;j<cols;j++)
  {
    ...
    {
      mask.at<uchar>(i,j) = img.at<uchar>(i,j)<tresh?255:0;
    }
   }
  mask=erDil(mask,1);
  return mask;
...
std::string DetectNotNormalized::checkBed(cv::Mat frame)
{
  int i,j,rows,cols;
  float xLeft, xRight;
  bool found;
...
cv::Mat DetectNotNormalized::tempDifference(cv::Mat prev, cv::Mat current)
{
  int i, j, rows,cols;
  cv::Mat movementMask;
...
cv::Mat DetectNotNormalized::createMaskNew(cv::Mat movementMask,cv::Mat frame)
{
  cv::Mat oneChannel;
  ...
  mask_open=DetectNotNormalized::erDil(mask,1);
  return mask_open;
}

cv::Mat DetectNotNormalized::erDil(cv::Mat img, int size)
{
...
cv::Mat DetectNotNormalized::tempDifferenceNew(cv::Mat prev, cv::Mat current)
{
  int i, j, rows,cols;
  int TRESH = 5;
